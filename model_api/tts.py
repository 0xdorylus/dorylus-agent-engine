...
async def local_tts_audio_stream(text, voice):
    ...
    async with aiohttp.ClientSession(
        # headers={'Authorization': 'Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJleHAiOjE3MTU1OTcwMDksImlhdCI6MTcxNTU5MzQwOSwicHVibGljX2tleSI6IkREdG83N2ZhZUoxdVNLUFJnbmFiS1ZwY3p6TGZqc1ZZUGFHbTdiWHlaTmdQIn0.VVm_B1YDdW1B4czpob_casDVH3aVV__KQgeCxG28ktU'}
    ) as session:
        payload = {
            "cha_name": voice,
...
async def play_audio_stream(text, voice):
    ...
    async for audio_data in async_audio_stream(text, voice):
        # send audio data from stream to ffplay
...
def play_audio_file_in_stream(file_path, block=True):
    ...
    with open(file_path, 'rb') as fp:
        # send audio data from file to ffplay
        while True:
...
async def fetch_audio_chunks_from_stream(text, voice):
    if USING_LOCAL_TTS:
        async_audio_stream = local_tts_audio_stream
    else:
        async_audio_stream = edge_tts_audio_stream
...
async def async_fetch_audio_chunks_from_file(file_path):
    chunk_size = 1024
...
def fetch_audio_chunks_from_file(file_path):
    chunk_size = 1024
    with open(file_path, 'rb') as fp:
        # read audio data
        while True:
...
async def async_fetch_const_audio_stream(key: str):
    if key not in CONST_SPEECH_MAP:
        return
    const_speech = CONST_SPEECH_MAP[key]
    
    file_exists = await aiofiles.ospath.exists(const_speech['file'])
    if not file_exists:
        return
    
    async for chunks in async_fetch_audio_chunks_from_file(const_speech['file']):
        yield chunks

async def fetch_const_audio_stream(key: str):
    if key not in CONST_SPEECH_MAP:
        return
    const_speech = CONST_SPEECH_MAP[key]
    
    file_exists = os.exists(const_speech['file'])
    if not file_exists:
        return
    
    for chunks in fetch_audio_chunks_from_file(const_speech['file']):
        yield chunks
